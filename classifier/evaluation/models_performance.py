...
figs_dir = 'classifier/fig'

#definición de métodos
#el otro
def grafica_roc_pr_area(y_tests, y_probas, seeds, model_type, color_graph):
    # ----------- CURVA ROC --------------
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(figs_dir, f'roc_curve_{model_type}_30_{color_graph}.png'), dpi=300, bbox_inches="tight", pad_inches=0.2)
    plt.close(fig)
    ...
    ax.grid(True, linestyle="--", alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(figs_dir, f"pr_curve_{model_type}_30_{color_graph}.png"), dpi=300, bbox_inches="tight", pad_inches=0.2)
    plt.close(fig)


#ubica el mejor threshold
def thresholds_PR(y_tests, y_probas, seeds, model_type, color_graph):    
    ...
    ax.grid(True, linestyle="--", alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(figs_dir, f'pr_curve_{model_type}_{seed}_{color_graph}_thresholds.png'), dpi=300, bbox_inches="tight", pad_inches=0.2)
...
def metricas(roc_scores, pr_scores, model_type):
    ...
    print(f"ROC AUC (media p5-p95): {mean_roc:.4f} ± {(2*std_roc):.6f}  usando {n_roc} valores y 2 desviaciones estándar")
    print(f"PR  AUC (media p5-p95): {mean_pr:.4f} ± {(2*std_pr):.6f}  usando {n_pr} valores y 2 desviaciones estándar")
    return mean_roc, (2*std_roc), n_roc, mean_pr, (2*std_pr), n_pr
...
def bloxplot_metricas(df_box): 
    ...
    plt.savefig(os.path.join(figs_dir, f'boxplot_auc_por_zona_y_modelo.png'), dpi=300, bbox_inches='tight')
...
def main(color_graph):
    ...
    for model_type, path in types.items():
        ...
        grafica_roc_pr_area(y_tests, y_probas, seeds, model_type, color_graph)
        thresholds_PR(y_tests, y_probas, seeds, model_type, color_graph) #temporal
        mean_roc, std_roc_2, n_roc, mean_pr, std_pr_2, n_pr= metricas(roc_scores, pr_scores, model_type)
        results[model_type] = {
            "ROC-AUC": (round(mean_roc,3), round(std_roc_2,3), n_roc),
            "PR-AUC":  (round(mean_pr,3), round(std_pr_2,3), n_pr),
        }
        ...
        for zona in ['atlantico', 'pacifico']:
            ...
            print(f"ROC AUC (media p5-p95): {mean_roc:.4f} ± {(2*std_roc):.6f}  usando {n_roc} valores, usando 2 desviaciones estándar")
            print(f"PR  AUC (media p5-p95): {mean_pr:.4f} ± {(2*std_pr):.6f}  usando {n_pr} valores, usando 2 desviaciones estándar")
